...
```{r}
ames_train%>%count(Sale.Condition
                   )
```


```{r}
(ames_train <- ames_train %>% dplyr::filter(Sale.Condition=="Normal"))
```
```{r}
ames_train
```
...
```
```{r}
ames_train %>% ggplot()+geom_bar(aes(x=Overall.Qual,stat=price))
```
```{r}
(model <- lm(price~Year.Built+Overall.Qual+area+Bedroom.AbvGr+Lot.Area+Overall.Cond,data = ames_train))
```
...
```
```{r}
stepAIC(model,k=2,direction = "backward",trace = FALSE)
```
...
```
```{r}
ames_train %>% dplyr::select(Year.Built,Overall.Qual , area , Bedroom.AbvGr ,
    Lot.Area ,Overall.Cond,price,)
```

```{r}
(df <- tibble(Year.Built=2010,Overall.Qual=as.character(5),area=900,Bedroom.AbvGr=4,Lot.Area=7654,Overall.Cond=as.character(5)))
```
